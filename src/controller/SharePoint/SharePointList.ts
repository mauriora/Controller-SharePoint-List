...
import { fixSingleTaxonomyFields, resultArrayToArray, setNullArrays, toSubmit } from './Transformer';
import { plainToClass, plainToClassFromExist } from 'class-transformer';
import { ODataError } from '../../models/OData/Error';
import { DataError } from '../../models/DataError';
// export { IFieldInfo } from "@pnp/sp/fields";
...
export const getLists = async (siteUrl?: string): Promise<Array<IListInfo>> => {
    const normalisedSiteUrl = siteUrl === undefined ? undefined : normaliseSiteUrl(siteUrl);
    const site: SiteInfo = siteUrl ? await getSite(normalisedSiteUrl) : getDefaultSite();

    if (undefined === site.lists) {
        site.lists = await site.web.lists.orderBy("Title")();
    }

    console.log(`controller/SharePoint/List:getLists(${siteUrl})`, { lists: site.lists });
...
export class SharePointList<DataType extends ListItemBase = ListItemBase> implements Controller<ListItemBase, DataType> {
    /**
    ...
    public records: Array<DataType> = new Array();
    ...
    public getById = async (id: number): Promise<DataType> => {
        ...
        try {
            const plain = this.getPlainById(id);

            const instance = await this.getObject(plain);
            this.records.push(instance);

            console.debug(`SharePointList[${this?.listInfo?.Title ?? this.listId ?? this.listTitle}].gotById(${id}) records=${this.records.length}`, { instance, plain, records: [... this.records], selects: this.selectFields, expands: this.expandFields });

            return instance;
        } catch (getItemsError) {
            throw new Error(`SharePointList[${this?.listInfo?.Title ?? this.listId ?? this.listTitle}].getById(${id}) failed: ${getItemsError.message ?? getItemsError}`);
        }
    }
     ...
     */
    public submit = async (jsRecord?: DataType): Promise<string | void> => {
        jsRecord = jsRecord ?? this.newRecord;

        const submitRecord = await toSubmit(jsRecord, this.selectedFields, this.allFields);
        ...
        if (undefined !== submitRecord.ID && 0 < submitRecord.ID) {
            ...
            console.log(`SharePointList[${this?.listInfo?.Title ?? this.listId ?? this.listTitle}].submit() update response.data.odata.etag=${updateResponse.data['odata.etag']}`, { jsRecord, submitRecord, updateResponse });
        } else {
    ...
    public constructor(public site: SiteInfo, listIdorTitle: string) {
        if (/^\{?[0-9a-f]{8}-[0-9a-f]{4}-[0-5][0-9a-f]{3}-[089ab][0-9a-f]{3}-[0-9a-f]{12}\}?$/i.test(listIdorTitle)) {
        ...
        this.context = site.isDefault ? site.context : {
            pageContext: {
                web: {
                    absoluteUrl: site.url,
                    language: getDefaultSite().context.pageContext.web.language
                }
            },
            spHttpClient: getDefaultSite().context.spHttpClient
        };
        console.log(`SharePointList( id=${this.listId}, title=${this.listTitle})`, { site, me: this, context: this.context, defaultContext: getDefaultSite().context });
    ...
    public get allFields(): Map<string, IFieldInfo> {
        if (0 === this._allFields.size) {
            throw new Error(`SharePointList[${this?.listInfo?.Title ?? this.listId ?? this.listTitle}].get allFields() not fields. Call getFieldInfos first`);
        }
    ...
    public init = async () => {
        try {
            ...
            throw new Error(`SharePointList[${this?.listInfo?.Title ?? this.listId ?? this.listTitle}].init(): ${this.listId ? 'getById' : 'getByTitle'}: ${getListError.message ?? getListError}`);
        }
        ...
        console.log(`SharePointList[${this?.listInfo?.Title ?? this.listId ?? this.listTitle}].init() done voting=${this.votingExperience}`, { site: this.site, listId: this.listId, allFields: this.allFields, selectedFields: this.selectedFields, list: this.list, selects: this.selectFields, expands: this.expandFields, me: this, rootFolderProperties: this.rootFolderProperties });
    }
    ...
    private async getFieldInfos() {
        try {
            ...
            throw new Error(`SharePointList[${this?.listInfo?.Title ?? this.listId ?? this.listTitle}].getFieldInfos() caught: ${getFieldsError.message ?? getFieldsError}`);
        }
    }
    ...
    private getListInfo = async () => {
        ...
        } catch (getListInfoError) {
            throw new Error(`SharePointList[${this?.listInfo?.Title ?? this.listId ?? this.listTitle}].getListInfo(): getList ${getListInfoError.message ?? getListInfoError}`);
        }
    }
    ...
    private getRootFolderProperties = async () => {
        try {
            this.rootFolderProperties = await this.list.rootFolder.properties.get();
            this.votingExperience = this.rootFolderProperties['Ratings_x005f_VotingExperience'] as undefined | 'Ratings' | 'Likes';
        } catch (getError) {
            throw new Error(`SharePointList[${this?.listInfo?.Title ?? this.listId ?? this.listTitle}].getRootFolderProperties(): getList ${getError.message ?? getError}`);
        }
    }
    ...
    private propertyFields = new Map<string, IFieldInfo>();
    ...
    private addModelSelectAndExpand = async (model: SharePointModel<DataType>) => {
        ...
        if (undefined === this.baseModel || this.baseModel.selectFields.length < model.selectFields.length) {
            console.debug(`SharePointList[${this?.listInfo?.Title ?? this.listId ?? this.listTitle}].addModelSelectAndExpand() set baseModel`, { model, controller: this });
            this.baseModel = model;
            model.newRecord = this.newRecord = await this.getObject();
        }
    }

    async addModel<ModelType extends DataType>(jsFactory: ListItemBaseConstructor<ModelType>, filter: string): Promise<SharePointModel<ModelType>> {
        const existing = this.models.get(jsFactory) as SharePointModel<ModelType>;

        console.debug(`SharePointList[${this?.listInfo?.Title ?? this.listId ?? this.listTitle}].addModel()`, { existing });
        ...
        return newModel as SharePointModel<ModelType>;
    }

    /**
     * Maps propertyname to { Lookup list Id, loadLookupController, thisFieldName }
     */
    private lookupMappings = new Map<string, LookupFieldMapping>();
                ...
                case FieldTypes.Lookup:
                    {
                        ...
                        if (undefined === lookUpListId) {
                            throw new Error(`SharePointList[${this?.listInfo?.Title ?? this.listId ?? this.listTitle}].createRequiredChildController no LookupList (ID) for ${property} of type ${info.TypeAsString}[${info.FieldTypeKind}]`);
                        } else if (!this.lookupMappings.has(property)) {
                            let lookupController = getById(lookUpListId, false);

                            if (undefined === lookupController) {
                                console.log(`SharePointList[${this?.listInfo?.Title ?? this.listId ?? this.listTitle}].createRequiredChildController create LookupList[${lookUpListId}] for ${property}`, { info });
                                lookupController = await getCreate(lookUpListId, this.site.url);
                                lookupController.init();
                            }
                            this.lookupMappings.set(
                                property,
                                {
                                    listId: lookUpListId,
                            ...
                            for (const model of this.models.values()) {
                                this.addLookupModeltoController(model, property, lookupController);
                            }
                        }
                    }
                    break;
    ...
    private addLookupModeltoController(model: SharePointModel<DataType>, property: string, lookupController: SharePointList<ListItemBase>) {
        const typeData = defaultMetadataStorage.findTypeMetadata(model.jsFactoryFactory(), property);

        if (!lookupController.models.has(typeData.typeFunction() as ListItemBaseConstructor)) {
            lookupController.addModel(typeData.typeFunction() as ListItemBaseConstructor, '');
        } else {
    ...
    public addGetPartial<T extends Partial<DataType> & ListItemBase>(item: T): T {
        ...
        if('function' === typeof( item.setController )) {
            ...
            console.error(`SharePointList[${this?.listInfo?.Title ?? this.listId ?? this.listTitle}].addGetPartial item has no setController`, {item, controller: this})
        }
    ...
    private loadFull = async (partial: Partial<DataType> & ListItemBase) =>
        this.getObject(
            await this.getPlainById(partial.id),
            partial
        );

    private static EXPANDED_FIELD_ERROR_500 = RegExp(/^Cannot get value for projected field ([A-z0-9_]+)_x005f_([A-z0-9_]+).$/);

    private static isExpanededFieldError500 = (error): string | undefined => {

        if (error.message && /error/.test(error.message)) {
            try {
                const errorObject = JSON.parse((error.message as string).substring((error.message as string).indexOf('{')));
                ...
                const errorMatches = SharePointList.EXPANDED_FIELD_ERROR_500.exec(content?.message?.value);

                if ((content?.code === '-2146232832, Microsoft.SharePoint.SPException') &&
                    (errorMatches.length >= 2)
                ) {
                    return errorMatches[1] + '/' + errorMatches[2];
                }
            } catch (parseErrorError) {
                throw new Error(`SharePointList.isExpanededFieldError500 JSON parse error: ${parseErrorError.message ?? parseErrorError}: parsing error: ${error?.message ?? error}`);
            }
        } else {
            console.debug('SharePointList.isExpanededFieldError500 no match', { error })
        }
        return undefined;
    }

    private static EXPANDED_FIELD_ERROR_400 = RegExp(/^The query to field '([A-z0-9_]+)\/([A-z0-9_]+)' is not valid.$/);

    private static isExpanededFieldError400 = (error): string | undefined => {

        // if(error.message && /\[500\]\s+::>\{"odata.error"/.test(error.message))  {
        if (error.message && /odata.error/.test(error.message)) {
            try {
                const odataError: ODataError = JSON.parse((error.message as string).substring((error.message as string).indexOf('{')));
                const errorMatches = SharePointList.EXPANDED_FIELD_ERROR_400.exec(odataError?.['odata.error']?.message?.value);

                console.debug(`SharePointList.isExpanededFieldError400 parsed`, { error, odataError, errorMatches });
                ...
                throw new Error(`SharePointList.isExpanededFieldError400 JSON parse error: ${parseErrorError.message ?? parseErrorError}: parsing error: ${error?.message ?? error}`);
            }
        } else {
    ...
    private handleFailedExpand = (expandedField: string) => {
        const fieldName = expandedField.substring(0, expandedField.indexOf('/'));
        const filteredSelects = Array.from(this.selectFields.values()).filter(selectField =>
            (!selectField.startsWith(fieldName + '/'))
        );
        filteredSelects.push(... (TAX_CATCH_ALL_FIELD === fieldName ? [fieldName + '/Term', fieldName + '/ID'] : [fieldName + '/Title', fieldName + '/ID']));

        if (filteredSelects.length !== this.selectFields.size) {
            console.warn(
                `SharePointList[${this?.listInfo?.Title ?? this.listId ?? this.listTitle}].handleFailedExpand removed ${this.selectFields.size - filteredSelects.length} because of field ${fieldName}.${expandedField}, load LookUp controller`,
                {
                    selectFields: [...this.selectFields],
                    filteredSelects: [...filteredSelects],
                    lookupMappings: this.lookupMappings,
                    lookupMappingsCount: this.lookupMappings.size,
                    lookupMappingsArray: Array.from(this.lookupMappings.entries())
                }
            );
            ...
            const [property, mapping] = Array.from(this.lookupMappings.entries()).find(([pProspect, mProspect]) => (mProspect.thisFieldName === fieldName));

            if (undefined === mapping) {
                throw new Error(`SharePointList[${this?.listInfo?.Title ?? this.listId ?? this.listTitle}].handleFailedExpand can't find mapping for expandedField ${fieldName}`)
            } else if (mapping.loadLookupController === true) {
                console.warn(`SharePointList[${this?.listInfo?.Title ?? this.listId ?? this.listTitle}].handleFailedExpand already loading lookup`)
            } else {
            ...
            throw new Error(`SharePointList[${this?.listInfo?.Title ?? this.listId ?? this.listTitle}].handleFailedExpand FAILED to remove ${expandedField} from [${Array.from(this.selectFields.values()).join(', ')}]`);
        }
    }
    ...
    public loadAllRecords = async (filter: string): Promise<void> => {
        try {
            ...
            console.debug(`SharePointList[${this?.listInfo?.Title ?? this.listId ?? this.listTitle}].getAll gotInstances records=${this.records.length}`, { plainItems, records: [... this.records] });
        } catch (getItemsError) {
            ...
            switch (getItemsError['status']) {
                ...
                default:
                    console.error(
                        `SharePointList[${this?.listInfo?.Title ?? this.listId ?? this.listTitle}].getAll failed allItmes=${this.records.length}: ${getItemsError.message ?? getItemsError}`,
                        { getItemsError, factory: this.baseModel.jsFactoryFactory(), selects: this.selectFields, expands: this.expandFields }
                    );
                    throw new Error(`SharePointList[${this?.listInfo?.Title ?? this.listId ?? this.listTitle}].getAll failed: ${getItemsError.message ?? getItemsError}`);
            }
            if (undefined !== failedExpandedField) {
                ...
                console.warn(`SharePointList[${this?.listInfo?.Title ?? this.listId ?? this.listTitle}].getAll removed ${failedExpandedField} and try again`, { getItemsError, failedExpandedField, selectFields: [...this.selectFields] });
     ...
     */
    public getNew = async () => this.getObject();
     ...
     */
    private getObject = async (plain?, existing?: Partial<DataType> & ListItemBase): Promise<DataType> => {
        if (plain) {
            console.warn(`SharePointList[${this?.listInfo?.Title ?? this.listId ?? this.listTitle}].getObject(${plain.ID}) resultArrayToArray`, { plainNow: { ...plain }, plain });

            resultArrayToArray(plain, this.selectedFields);
        }
        const instance = plain ?
            (existing ?
                plainToClassFromExist(existing, plain, { excludeExtraneousValues: true }) as DataType :
                plainToClass(this.baseModel.jsFactoryFactory(), plain, { excludeExtraneousValues: true }))
            :
            (existing as DataType ?? new (this.baseModel.jsFactoryFactory())());
        ...
        if (instance.id) {
            if (instance.pnpItem) {
                console.warn(`SharePointList[${this?.listInfo?.Title ?? this.listId ?? this.listTitle}].getObject(${instance.id}) instance.pnpItem already set - setting again`);
            }
        ...
        when(() => instance.deleted)
            .then(() => this.removeItem(instance));

        console.log(`SharePointList[${this?.listInfo?.Title ?? this.listId ?? this.listTitle}].getObject(${instance.id})`, { plain, existing, instance });
        return instance;
    }
    ...
    private connectLookUp = (item: DataType) => {
        for (const [property, mappingInfo] of this.lookupMappings) {
            ...
            if (undefined !== tempLookup) {
                if (undefined !== item.source[mappingInfo.thisFieldName]?.['__deferred'] && 'id' in tempLookup && undefined === tempLookup['id']) {
                    ...
                    if (true === fieldInfo['AllowMultipleValues']) {
    ...
    private removeItem = (item: DataType) => {
        const index = this.records.findIndex(prospect => prospect.id === item.id);
 ...
 */
export const create = async (listId: string, siteUrl?: string): Promise<SharePointList> => {
    ...
    try {
        const site = siteUrl === undefined ? getDefaultSite() : await getSite(normaliseSiteUrl(siteUrl));
        console.log(`SharePointList:create ( ${url} )`, { site, listId, siteUrl });
        ...
        throw new Error(`SharePointList:create( ${url} ) failed: ${initError.message ?? initError}`);
    }
};
 ...
 * @returns 
 */
export const getByUrl = (listIdOrName: string, siteUrl: string): SharePointList => {
    const url = getListUrl(listIdOrName, siteUrl);

    const controller = controllers.get(url);

    if (undefined === controller) {
        throw new Error(`SharePointList:getByUrl: Can't find controller for ${listIdOrName} at ${siteUrl} = ${url}. Call create first. Got ${controllers.size} controllers: ${(new Array(controllers.keys())).join(' ; ')}`);
    }
 ...
 * @returns 
 */
export const getById = (listGuid: string, throwExceptionIfDoesntExist: boolean = true): SharePointList => {
    const controller = Array.from(controllers.values()).find(prospect => (prospect.listId === listGuid));

    if (throwExceptionIfDoesntExist && (undefined === controller)) {
        throw new Error(`SharePointList:get: Can't find controller for ${listGuid}.Got ${controllers.size} controllers: ${(Array.from(controllers.values())).map(item => item.listId).join(' ; ')}`);
    }

    return controller;
};

export const getCreate = async (listId: string, siteUrl?: string): Promise<SharePointList> => {
...
export const getUserLookupSync = (userId: number, info: IFieldInfo): UserLookup => {
    const lookUpListId = info['LookupList'];

    if (undefined === lookUpListId)
        throw new Error(`SharePointList getUserLookupSync no LookupListID in (field)info`);
